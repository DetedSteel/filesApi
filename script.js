			...
			dropArea.addEventListener("drop", (e) => {
				...
				[...transferredFiles].forEach((transferredFile) => {
					...
					reader.addEventListener("error", () => {
						console.error(
							`Произошла ошибка при чтении файла: ${transferredFile.name}`
						);
						return;
					});
...
function insertHTML(name, size) {
  dropArea.insertAdjacentHTML(
    "beforeend",
    `<div class="drop-area-preview"><div class="drop-area-name">File name: ${name}</br>File size: ${(size / 1024).toFixed(2)}KB</div><div id="${name.match(/[a-zA-Z0-9]/g).join('')}" class="drop-area-remove"><svg viewBox="0 0 24 24" height="24" width="24"><path d="M0 0h24v24H0z" fill="none"></path><path fill='#f5f5f5' d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"></path></svg></div>
...
`<select name="s" id="selectType"><option value="All">All</option></select>
